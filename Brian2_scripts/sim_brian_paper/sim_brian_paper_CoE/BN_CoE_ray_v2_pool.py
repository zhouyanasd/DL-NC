...
data_path = project_dir_sever+'/Data/20bn-jester-v1/'
LHS_path = exec_dir+'/LHS_BN.dat'
...
try:
    ...
    df_en_train = BN.encoding_latency_BN(df_train, origin_size, pool_size, pool_types, pool_threshold, False)
    ...
    df_en_pre_train = BN.encoding_latency_BN(df_pre_train, origin_size, pool_size, pool_types, pool_threshold, False)
    ...
    df_en_validation = BN.encoding_latency_BN(df_validation, origin_size, pool_size, pool_types, pool_threshold, False)
    ...
    df_en_test = BN.encoding_latency_BN(df_test, origin_size, pool_size, pool_types, pool_threshold, False)
...
data_validation_index_batch = BN.data_batch(df_en_validation.index.values, core)
data_test_index_batch = BN.data_batch(df_en_test.index.values, core)
...
def pre_run_net(gen, data_index):
    exec(exec_env)
    exec(exec_var)
    BN_ = BN_classification()
    df_en_pre_train = BN_.load_data(data_path + 'pre_train_' + DataName + '.p')
...
def run_net(gen, state_pre_run, data_indexs):
    ...
    exec(exec_var)
    BN_ = BN_classification()
    df_en_train = BN_.load_data(data_path + 'train_' + DataName+'.p')
    df_en_validation = BN_.load_data(data_path + 'validation_' + DataName+'.p')
    df_en_test = BN_.load_data(data_path + 'test_' + DataName+'.p')
