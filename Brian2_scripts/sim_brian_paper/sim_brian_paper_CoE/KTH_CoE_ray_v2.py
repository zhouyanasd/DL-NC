...
"""
    ...
    This version use ray to speed up in the ray cluster.
...
def parameters_search(**parameter):
    ...
    [pre_run_net.remote(gen, x, q) for x in zip(data_pre_train_s, label_pre_train)]
    sum_strength(gen, q)
    # ------parallel run for train-------
    states_train_list = ray.get([run_net.remote(gen, (x)) for x in zip(data_train_s, label_train)])
    # ------parallel run for validation-------
    states_validation_list = ray.get([run_net.remote(gen, (x)) for x in zip(data_validation_s, label_validation)])
    # ----parallel run for test--------
    states_test_list = ray.get([run_net.remote(gen, (x)) for x in zip(data_test_s, label_test)])
