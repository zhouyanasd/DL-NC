...
data_path = project_dir_sever + '/Data/N_MNIST/'
LHS_path = exec_dir + '/LHS_N_MNIST.dat'
...
neurons_encoding = 2 * shape[0] * shape[1]

##-----------------------------------------------------
# --- data and classifier ----------------------
NMNIST = NMNIST_classification(shape, coding_duration)
...
# -------data initialization----------------------
try:
    df_en_train = NMNIST.load_data(data_path + 'train_' + DataName+'.p')
    df_en_pre_train = NMNIST.load_data(data_path + 'pre_train_' + DataName + '.p')
    df_en_validation = NMNIST.load_data(data_path + 'validation_' + DataName+'.p')
    df_en_test = NMNIST.load_data(data_path + 'test_' + DataName+'.p')
except FileNotFoundError:
    NMNIST.load_data_NMNIST_all(data_path)
    ...
    NMNIST.dump_data(data_path + 'train_' + DataName + '.p', df_en_train)
    NMNIST.dump_data(data_path + 'pre_train_' + DataName + '.p', df_en_pre_train)
    NMNIST.dump_data(data_path + 'validation_' + DataName + '.p', df_en_validation)
    NMNIST.dump_data(data_path + 'test_' + DataName + '.p', df_en_test)

data_train_index_batch = NMNIST.data_batch(df_en_train.index.values, core)
data_pre_train_index_batch = NMNIST.data_batch(df_en_pre_train.index.values, core)
data_validation_index_batch = NMNIST.data_batch(df_en_validation.index.values, core)
data_test_index_batch = NMNIST.data_batch(df_en_test.index.values, core)
