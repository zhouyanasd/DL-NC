...
class MNIST_classification():
    def __init__(self, shape):
        self.shape = shape
    def load_Data_MNIST(self,n, path_value, path_label):
    ...
    def encoding_latency_MNIST(self, analog_data, n, duration, min=0, max=np.pi, *args):
    ...
    def get_series_data(self, n, data_frame, duration, is_order=True, *args, **kwargs):
        try:
            obj = kwargs['obj']
        except KeyError:
            obj = np.arange(10)
        ...
        data_frame_s = []
        for value in self.encoding_latency_MNIST(data_frame_obj['value'][:n], kwargs['coding_n'], int(0.1 * duration)):
            for data in value:
