...
# Results for ST-Coding MNIST in LSM with STDP
# Input is 28*28
#----------------------------------------------
...
def LSM_with_STDP(seed):
    result = simulate_LSM(seed, pre_train_loop = 1)
    return result
...
if __name__ == '__main__':
    ...
    df_train.to_csv('../../Data_result/JV_STDP_statistic/train_without_STDP_v1281.csv',index = False)
    df_test.to_csv('../../Data_result/JV_STDP_statistic/test_without_STDP_v1281.csv',index = False)

    result = p.map(LSM_with_STDP, np.arange(tries))
    sta_data_tri, sta_data_test = [x[0] for x in result], [x[1] for x in result]
    df_train, df_test = pd.DataFrame(np.asarray(sta_data_tri)), pd.DataFrame(np.asarray(sta_data_test))
    df_train.to_csv('../../Data_result/JV_STDP_statistic/train_with_STDP_v1281.csv', index=False)
    df_test.to_csv('../../Data_result/JV_STDP_statistic/test_with_STDP_v1281.csv', index=False)


    # ------results data ----
    data_train_without = pd.read_csv("../../Data_result/JV_STDP_statistic/train_without_STDP_v1281.csv")
    data_test_without = pd.read_csv("../../Data_result/JV_STDP_statistic/test_without_STDP_v1281.csv")
    data_train_with = pd.read_csv("../../Data_result/JV_STDP_statistic/train_with_STDP_v1281.csv")
    data_test_with = pd.read_csv("../../Data_result/JV_STDP_statistic/test_with_STDP_v1281.csv")
