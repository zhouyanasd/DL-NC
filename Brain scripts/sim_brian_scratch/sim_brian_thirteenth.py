...
def XOR_classification(neu =2, interval_l=8,interval_s = ms):
    def tran_XOR(A):
        trans = []
        for a in A:
            for i in range(4):
    ...
    label_a = np.random.randint(0,2,n)
    seq_a = tran_XOR(label_a)
    times_a = where(seq_a ==1)[0]
    indices_a = zeros(int(len(times_a)))

    label_b = np.random.randint(0,2,n)
    seq_b = tran_XOR(label_b)
    times_b = where(seq_b ==1)[0]
    indices_b = zeros(int(len(times_b)))+1

    indices = np.concatenate((indices_a,indices_b),axis=0)
    times = np.concatenate((times_a, times_b), axis=0)*interval_s
    label = np.logical_xor(label_a,label_b).astype(int)
...
P , label = XOR_classification(2,interval_l,interval_s)
...
subplot(111)
plt.scatter(m1.t[1:] / ms, label_t_class, s=6, color="red", marker='o',alpha=0.6)
plt.scatter(m1.t[1:] / ms, label, s=6, color="blue",marker='v',alpha=0.6)
plt.scatter(m1.t[1:] / ms, data_n, s=3, color="green")
axhline(threshold, ls='--', c='r', lw=2)
